...
void videoBackground(struct Graphics* gr){
    SDL_Rect rects[3];
    rects[0] = (SDL_Rect){ .x = 0, .y = 0, .w = 160, .h = 720 };
    rects[1] = (SDL_Rect){ .x = 160, .y = 0, .w = 960, .h = 720 };
    rects[2] = (SDL_Rect){ .x = 1120, .y = 0, .w = 160, .h = 720 };
    
    SDL_FillRect(gr->window_surface, &rects[0], SDL_MapRGBA(gr->window_surface->format, 0x00, 0x00, 0x00, 0xFF));
    SDL_FillRect(gr->window_surface, &rects[1], SDL_MapRGBA(gr->window_surface->format, 0xFF, 0xFF, 0xFF, 0xFF));
...
void playVideo(struct Graphics* gr, PadState* pad, int xi, int yi)
{
	...
	for(int i=0; i<5299; i++){
		...
		const unsigned long long* bitmap = bad_apple_frames[i];
